...
Program Definition I_fix_x {A : Type} (P : A → list Type) (f : IRel_x P → IRel_x P)
  ...
  IRel_x_curry _ (I_fix_2 
    (λ R, IRel_x_uncurry _ (f (IRel_x_curry _ R)))
    (Private_x.contractive_x_conv _ f H)
  ).
...
Lemma contractive_xx_curry {A B : Type} (P : A → B → list Type) :
  ∀ (f : IRel_xx P → IRel_xx P), contractive_xx P f →
  contractive_xx (λ x y, Prod (P x y) :: nil)
    (λ R, IRel_xx_uncurry _ (f (IRel_xx_curry _ R))).
Proof.
  unfold contractive_xx.
  intros f Hcon R₁ R₂ n.
  iintro HR.
  iapply (IRel_xx_uncurry_equiv _ (f (IRel_xx_curry _ R₁)) (f (IRel_xx_curry _ R₂)) n). 
  iapply (Hcon (IRel_xx_curry _ R₁) (IRel_xx_curry _ R₂) n).
  later_shift.
  iapply (IRel_xx_curry_equiv _ R₁ R₂ n); assumption.
Qed.

Lemma contractive_binary {A B : Type} {P : A → B → Type} (f : IRel_3 P → IRel_3 P) :
  contractive_xx (λ x y, P x y :: nil) f → contractive_3 f.
Proof.
  unfold contractive_xx; unfold contractive_3.
  intros Hcon n R₁ R₂ H x y z.
  specialize (Hcon R₁ R₂ n); ispecialize Hcon.
  + later_shift.
    iintro x' ; iintro y' ; iintro z'.
    apply I_iff_intro_M; intros; apply H.
    apply le_lt_n_Sm; assumption.
  + simpl in Hcon.
    apply I_iff_elim_M.
...
Lemma contractive_xx_conv {A B : Type} (P : A → B → list Type) (f : IRel_xx P → IRel_xx P) :
  contractive_xx P f → contractive_3 (λ R, IRel_xx_uncurry _ (f (IRel_xx_curry _ R))).
Proof.
  intros; apply contractive_binary; apply contractive_xx_curry; assumption.
Qed.

End Private_xx.

Program Definition I_fix_xx {A B : Type} (P : A → B → list Type) (f : IRel_xx P → IRel_xx P)
    (H : contractive_xx P f) : IRel_xx P :=
  IRel_xx_curry _ (I_fix_3 
    (λ R, IRel_xx_uncurry _ (f (IRel_xx_curry _ R)))
    (Private_xx.contractive_xx_conv _ f H)
  ).

Lemma I_fix_xx_unroll {A B : Type} (P : A → B → list Type)
    (f : IRel_xx P → IRel_xx P) (H : contractive_xx _ f) :
  subrel_xx _ (I_fix_xx _ f H) (f (I_fix_xx _ f H)).
Proof.
  unfold I_fix_xx; simpl.
  apply IRel_xx_uncurry_subrel; simpl ; intros x y z n H'.
  apply (I_fix_3_unroll (λ R, IRel_xx_uncurry _ (f (IRel_xx_curry _ R)))).
  apply IRel_uncurry_curry_id; assumption.
Qed.

Lemma I_fix_xx_roll {A B : Type} (P : A → B → list Type)
    (f : IRel_xx P → IRel_xx P) (H : contractive_xx _ f) :
  subrel_xx _ (f (I_fix_xx _ f H)) (I_fix_xx _ f H).
Proof.
  unfold I_fix; simpl.
  apply IRel_xx_uncurry_subrel; simpl; intros x y z n H'.
  apply IRel_uncurry_curry_id.
  apply (I_fix_3_roll (λ R, IRel_xx_uncurry _ (f (IRel_xx_curry _ R)))).
  assumption.
Qed.
